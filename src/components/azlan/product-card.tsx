...
import { ItemReviewsModal } from "./item-reviews-modal";
    ...
    <>
      ...
      >
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          {/* Meta & Interactive Review trigger */}
          <div className="mt-2 flex items-center justify-between text-[11px] text-[var(--color-on-surface-variant)]">
            <button
              onClick={() => setShowReviewsModal(true)}
              className="inline-flex items-center gap-1 font-semibold text-amber-600 hover:text-amber-700 hover:underline transition-colors cursor-pointer"
            >
              <span
              ...
              <span>{hasReviews ? `${Number(item.rating).toFixed(1)} (${item.review_count})` : "Rate Item"}</span>
            </button>
      ...
      {/* Item Reviews Modal */}
      {showReviewsModal && (
        <ItemReviewsModal item={item} onClose={() => setShowReviewsModal(false)} />
      )}
    </>
  );
}
